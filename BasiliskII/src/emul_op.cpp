	...
	switch (opcode) {
		...
		default:
			if (opcode == M68K_EMUL_OP_FIX_DISPATCH_MAGIC) {
				...
				// Ensure $02AE (ROMBase) is set.
				// The trap table builder at 0x809A9A loads A3 from ($02AE).W.
				// If $02AE is 0, all trap table entries are wrong.
				if (ReadMacInt32(0x02AE) == 0)
					WriteMacInt32(0x02AE, ROMBaseMac);
				// Save SR (replaces original MOVE.W SR,($0C74).W)
