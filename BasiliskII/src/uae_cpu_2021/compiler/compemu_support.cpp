...
#if defined(CPU_AARCH64)
static uae_u32 jit_mem_pre_regs[16];
static uae_u32 jit_mem_pre_nzcv, jit_mem_pre_x;
static uae_u8* jit_mem_pre_pc_p;

extern "C" void jit_mem_save_pre(uae_u32 pc, uae_u32 opcode)
{
    for (int i = 0; i < 16; i++)
        jit_mem_pre_regs[i] = regs.regs[i];
    jit_mem_pre_nzcv = regflags.nzcv;
    jit_mem_pre_x = regflags.x;
    jit_mem_pre_pc_p = regs.pc_p;
}
...
extern "C" void jit_watch_mem(uae_u32 pc, uae_u32 opcode)
{
    ...
    if (verified > 5000) return;
    verified++;
    
    /* Save compiled output (registers + memory snapshot) */
    uae_u32 comp_regs[16];
    for (int i = 0; i < 16; i++) comp_regs[i] = regs.regs[i];
    uae_u32 comp_nzcv = regflags.nzcv;
    uae_u8* comp_pc_p = regs.pc_p;
    
    /* Snapshot memory around destination (use pre-state A0 and immediate) */
    uae_u8 comp_mem[32];
    uae_u8 comp_mem[32]; for (int i = 0; i < 32; i++) comp_mem[i] = get_byte(0x3200 + i);
    
    /* Restore pre-state */
    for (int i = 0; i < 16; i++)
        regs.regs[i] = jit_mem_pre_regs[i];
    regflags.nzcv = jit_mem_pre_nzcv;
    regflags.x = jit_mem_pre_x;
    regs.pc_p = jit_mem_pre_pc_p;
    ...
    bool mem_diff = false;
    for (int i = 0; i < 32; i++) {
        uae_u8 interp_byte = get_byte(0x3200 + i);
        if (interp_byte != comp_mem[i]) {
            if (!mem_diff) {
                fprintf(stderr, "MEMVERIFY[%d] MISMATCH pc=%08x op=%04x\n", verified, pc, opcode);
                mem_diff = true;
                mismatches++;
            }
            fprintf(stderr, "  addr=%04x compiled=%02x interp=%02x\n",
                0x3200 + i, comp_mem[i], interp_byte);
        }
    }
    
    /* Also compare registers */
    for (int i = 0; i < 16; i++) {
        if (comp_regs[i] != regs.regs[i] && !mem_diff && mismatches == 0) {
            fprintf(stderr, "MEMVERIFY[%d] REG MISMATCH pc=%08x op=%04x reg[%d] comp=%08x interp=%08x\n",
                verified, pc, opcode, i, comp_regs[i], regs.regs[i]);
            mismatches++;
        }
    }
    
    /* Restore compiled results */
    for (int i = 0; i < 16; i++)
        regs.regs[i] = comp_regs[i];
    regflags.nzcv = comp_nzcv;
    regs.pc_p = comp_pc_p;
    
    /* Also restore the compiled memory state (undo interpreter writes) */
    /* Actually, we need to keep the COMPILED memory state, so undo
       the interpreter's writes by restoring from snapshot: */
    for (int i = 0; i < 32; i++)
        put_byte(0x3200 + i, comp_mem[i]);
    
    if (verified <= 10 && !mem_diff) {
        fprintf(stderr, "MEMVERIFY[%d] OK pc=%08x op=%04x\n", verified, pc, opcode);
    }
}
#endif
