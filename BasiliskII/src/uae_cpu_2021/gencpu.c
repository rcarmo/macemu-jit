...
static void generate_functbl (void)
{
	...
	fprintf(dispatchmapfile, "/* Auto-generated by gencpu.c. */\n");
	fprintf(dispatchmapfile, "#ifndef THREADED_DISPATCH_MAP_ENTRY\n");
	fprintf(dispatchmapfile, "#define THREADED_DISPATCH_MAP_ENTRY(opcode, label, handler)\n");
	fprintf(dispatchmapfile, "#endif\n\n");
	fprintf(dispatchmapfile, "#if !defined(HAVE_GET_WORD_UNSWAPPED) || defined(FULLMMU)\n");
	for (opcode = 0; opcode < 65536; opcode++)
	{
		fprintf(dispatchmapfile, "THREADED_DISPATCH_MAP_ENTRY(0x%04x, threaded_op_%04x, %s)\n", opcode, opcode, cpufunctbl[opcode].handler);
	}
	fprintf(dispatchmapfile, "#else\n");
	for (opcode = 0; opcode < 65536; opcode++)
	{
		unsigned int map = do_byteswap_16(opcode);
		fprintf(dispatchmapfile, "THREADED_DISPATCH_MAP_ENTRY(0x%04x, threaded_op_%04x, %s)\n", opcode, opcode, cpufunctbl[map].handler);
	}
	fprintf(dispatchmapfile, "#endif\n\n");
	fprintf(dispatchmapfile, "#undef THREADED_DISPATCH_MAP_ENTRY\n");
...
int main(void)
{
    ...
    if ((functblfile = fopen ("cpufunctbl.cpp", "wb")) == NULL)
    	abort(); 
	if ((dispatchmapfile = fopen ("threaded_dispatch_map.inc", "wb")) == NULL)
	    abort();
