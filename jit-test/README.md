...
`run.sh` currently covers 128 vectors across:
...
- Condition-byte writes via `Scc` families (`st/sf`, `shi/sls`, `scc/scs`, `sne/seq`, `svc/svs`, `spl/smi`, `sge/slt`, `sgt/sle`) plus CCR-preservation interactions (`SLT`→`BLT`, `SCS`→`BCS`, `SNE`→`BNE` with unchanged flags)
