...
`run.sh` currently covers 187 vectors across:
...
- Condition-byte writes via `Scc` families (`st/sf`, `shi/sls`, `scc/scs`, `sne/seq`, `svc/svs`, `spl/smi`, `sge/slt`, `sgt/sle`) plus expanded CCR-preservation interactions (`SLT`→`BLT`, `SCS`→`BCS`, `SNE`→`BNE`, `SEQ`→`BEQ`, `SVS`→`BVS`, `SVC`→`BVC`, `SHI`→`BHI`, `SLS`→`BLS`, `SPL`→`BPL`, `SMI`→`BMI`, `SGE`→`BGE`, `SGT`→`BGT` with unchanged flags)
